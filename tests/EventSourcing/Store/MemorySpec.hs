...
spec = do
  ...
  context "when a few events are inserted into the TVar store" $ do
    store <- runIO eventStoreMapTVar
  ...
  context "when events from multiple UUIDs are inserted into the TVar store" $ do
  ...
  -- IORef
  context "when the IORef memory event store is empty" $ do
    store <- runIO eventStoreMapIORef

    it "shouldn't have UUIDs" $ do
      getUuids store `shouldReturn` []

  context "when a few events are inserted into the IORef store" $ do
    store <- runIO eventStoreMapIORef
    let events = [Added 1, Added 4, Added (-3)]
    _ <- runIO $ storeSerializedEvents store nil events
  ...
  context "when events from multiple UUIDs are inserted into the IORef store" $ do
    let uuid1 = uuidFromInteger 1
        uuid2 = uuidFromInteger 2
    store <- runIO eventStoreMapIORef
    _ <- runIO $ do
